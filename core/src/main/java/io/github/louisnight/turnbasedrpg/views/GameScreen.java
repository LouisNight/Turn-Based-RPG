...
public class GameScreen implements Screen {
   ...
   private void loadCollisionRectangles() {
    ...
    public void removeDefeatedEnemy(Enemy defeatedEnemy) {
        enemies.remove(defeatedEnemy);  // Remove the enemy from the list
        System.out.println("Enemy removed from overworld.");
    }
    ...
    private void centerCameraOnMap() {
            ...
            public void render(float delta) {
                ...
                for (Enemy enemy : enemies) {
                    Rectangle enemyBoundingBox = enemy.getBoundingBox();  // Assuming each enemy has a bounding box
    ...
    public void update(float delta) {
        ...
        for (Enemy enemy : enemies) {
            // Check if player and enemy collide, and ensure no transition is happening
            if (player.getHitbox().overlaps(enemy.getHitbox()) && !screenTransition.isTransitioning()) {
                // Trigger screen transition for combat only between player and enemy
                screenTransition.startFadeOut();
                switchToCombat = true;
                collidedEnemy = enemy;  // Store the enemy involved in the collision
            }
        }

        // Handle transition to combat screen after fade out
        if (!screenTransition.isFadingOut() && !screenTransition.isFadingIn() && switchToCombat) {
            switchToCombatScreen();  // Switch to combat screen
